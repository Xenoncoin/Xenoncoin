...
// Copyright (c) 2014 The Xenoncoin developers
...
const string strMessageMagic = "Xenoncoin Signed Message:\n";
...
static const int64 nMinSubsidy = 1.25 * COIN;
...
static const int64 nMaxAdjustUp = 5; // 2% adjustment up
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XenoncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XenoncoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("Xenoncoin miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        printf("Xenoncoin miner terminated\n");
        throw;
    }
}
